...
import (
	...
	cls "github.com/weaviate/weaviate/modules/reranker-custom/ent"
...
func (c *client) performRank(ctx context.Context,
	...
	req, err := http.NewRequestWithContext(ctx, "POST", c.url("/rerank", cfg),
...
func (c *client) url(path string, cfg moduletools.ClassConfig) string {
	url := c.origin

	ichek := cls.NewClassSettings(cfg)
